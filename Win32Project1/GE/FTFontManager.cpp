...
DECLARE_SINGLETON_MEMBER(FTFontManager)
FTFontManager::CFont::CFont()
{
	m_face = NULL;
	m_rgba = NULL;
	m_antialias = false;
...
bool FTFontManager::CFont::create(FT_Library library, const char *filename, FT_Long face_index, int tall, bool bold, bool italic, bool antialias)
{
	FT_Error err;
...
void FTFontManager::CFont::release(void)
{
	FT_Error err;
...
bool FTFontManager::CFont::getCharInfo(int code, glyphMetrics *metrics, GLuint *texture, float coords[])
{
	// fast find it
...
int FTFontManager::CFont::getFontTall(void)
{
	return m_tall;
}
...
bool FTFontManager::CFont::loadChar(int code, glyphMetrics *metrics)
{
	FT_Error err;
...
// CChar
//------------------------------------------------------------
void FTFontManager::CFont::CChar::setInfo(glyphMetrics *metrics)
{
	memcpy(&m_metrics, metrics, sizeof(glyphMetrics));
}

void FTFontManager::CFont::CChar::getInfo(glyphMetrics *metrics, GLuint *texture, float coords[])
{
...
// CPage
//------------------------------------------------------------
FTFontManager::CFont::CPage::CPage()
{
	m_wide = m_tall = 256;
	m_posx = m_posy = 0;
...
bool FTFontManager::CFont::CPage::append(int wide, int tall, GLubyte *rgba, float coords[])
{
...
GLuint FTFontManager::CFont::CPage::getTexture(void)
{
	return m_texture;
}
...
bool FTFontManager::initialize(void)
{
	FT_Error err;
...
void FTFontManager::release(void)
{
	FT_Error err;
...
int FTFontManager::createFont(const char *filename, int face, int tall, bool bold, bool italic, bool antialias)
{
...
bool FTFontManager::getCharInfo(int font_index, int code, int *wide, int *tall, int *horiBearingX, int *horiBearingY, int *horiAdvance, GLuint *texture, float coords[])
{
...
int FTFontManager::getFontTall(int font_index)
{
	int i = CONVERT_FONT_INDEX(font_index);
